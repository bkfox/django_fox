...
class CapabilityQuerySet(models.QuerySet):
    ...
    def get_or_create_many(
        self, items: Iterable[Capability]
    ) -> models.Queryset:
...
class Capability(models.Model):
    ...
    Provide permission for a specific action. Capability are stored as
    unique for each action/max_derive couple.
    """
    ...
    def derive(self, max_derive: Union[None, int] = None) -> Capability:
        ...
        if not self.can_derive(max_derive):
            raise PermissionDenied(
                __("can not derive capability {name}").format(name=self.name)
            )
    ...
    def is_derived(self, capability: Capability = None) -> bool:
        ...
        return self.name == capability.name and self.can_derive(
            capability.max_derive
        )
