...
class Agent(models.Model):
    """An agent is the one that actually execute an action.

    It can either be a user or a group.
    """

    ref = models.UUIDField(_("Public Reference"), default=uuid.uuid4)
    """Public reference to agent."""
    user = models.ForeignKey(
        User, models.CASCADE, null=True, blank=True, verbose_name=_("User")
    )
    """Agent targets this user."""
    group = models.ForeignKey(
        Group, models.CASCADE, null=True, blank=True, verbose_name=_("Group")
    )
    """Agent targets this group."""
    is_default = models.BooleanField(
        _("Default User Agent"), default=False, blank=True
    )
    ...
    def clean(self):
        ...
        if self.is_default and not (self.user or self.is_anonymous):
            raise ValidationError(
                _("Agent can be set as default only when " "targeting user.")
            )
        super().clean()
