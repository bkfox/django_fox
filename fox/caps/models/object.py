    ...
    def get_reference_class(cls, name, bases, attrs):
        ...
        if not reference_class:
            bases = (
                b
                for b in bases
                if hasattr(b, "_meta") and not b._meta.abstract
            )
            items = (
                r
                for r in (getattr(b, "Reference", None) for b in bases)
                if issubclass(r, Reference)
            )
            reference_class = next(items, None)
        elif not issubclass(reference_class, Reference):
            raise ValueError(
                "{} Reference member must be a subclass "
                "of `fox.caps.models.Reference`".format(name)
            )
        return reference_class

    def __new__(cls, name, bases, attrs, **kwargs):
        reference_class = cls.get_reference_class(name, bases, attrs)
        new_class = super(ObjectBase, cls).__new__(
...
class ObjectQuerySet(models.QuerySet):
    ...
    def _select_references(
        self, refs_queryset: models.QuerySet
    ) -> ObjectQuerySet:
...
class Object(models.Model, metaclass=ObjectBase):
    ...
    `fox.caps.models.Reference`) that is used as object's specific
    ...
    def reference(self):
        ...
        return (
            self._agent_reference_set and self._agent_reference_set[0] or None
        )
